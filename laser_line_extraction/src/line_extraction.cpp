...
void LineExtraction::mergeLines()
{
  ...
  for (std::size_t i = 1; i < lines_.size(); ++i)
  {
    ...
    {
      // // Get L, P_1, P_2 of consecutive lines
      Eigen::Vector2d L_1(lines_[i].getRadius(), lines_[i].getAngle());
      Eigen::Vector2d L_2(lines_[zeroth_merge].getRadius(), lines_[zeroth_merge].getAngle());
      Eigen::Matrix2d P_1;
      P_1 << lines_[i].getCovariance()[0], lines_[i].getCovariance()[1],
            lines_[i].getCovariance()[2], lines_[i].getCovariance()[3];
      Eigen::Matrix2d P_2;
      P_2 << lines_[zeroth_merge].getCovariance()[0], lines_[zeroth_merge].getCovariance()[1],
            lines_[zeroth_merge].getCovariance()[2], lines_[zeroth_merge].getCovariance()[3];
      Eigen::Vector2d diff = L_1 - L_2;
      if (diff[1] > 1.57){
        L_1[1] -= 3.1416;
      }
      // ROS_DEBUG("zeroth %d %d ", zeroth_push, zeroth_merge);
      ...
      if (chiSquared(L_1-L_2, P_1, P_2) < 1000)
      {
        ...
        cov[3] = P_m(1,1);
        std::vector<unsigned int> indices;
        const std::vector<unsigned int> &ind_1 = lines_[i].getIndices();
        const std::vector<unsigned int> &ind_2 = lines_[zeroth_merge].getIndices();
        indices.resize(ind_1.size() + ind_2.size());
        indices.insert(indices.end(), ind_2.begin(), ind_2.end());
        indices.insert(indices.end(), ind_1.begin(), ind_1.end());
